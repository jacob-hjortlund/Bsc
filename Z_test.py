...
kernel_info = {'RBF': {'ndims': 2, 'kernel': rbf, 'logprior': rbf_logprior, 'inisamples': rbf_inisamples, 'samples': rbf_samples}, 
			   ...
			   'Matern': {'ndims': 2, 'kernel': matern, 'logprior': matern_logprior, 'inisamples': matern_inisamples, 'samples': matern_samples}}
...
# set additional args for the posterior (the data, log prior, and the kernel)
#argslist = (data, rbf_logprior, rbf)
...
def Z_est(n):
    ...
    z_tmp = np.exp(loglikelihood(theta_samples[n], data, kernel=kernel)-1500)
    z_sq_tmp = np.exp(2*loglikelihood(theta_samples[n], data, kernel=kernel)-3000)
    
    res_tmp = np.array([z_tmp, z_sq_tmp])
...
with MPIPool() as pool:
    ...
    likelihood_vals = np.array(pool.map(Z_est, range(n)))
...
np.save(path + f'/{kernel_name}_Z.npy', np.array([Z_val, Z_err]))
...
# np.save('./pulsar_results/%s_%s.npy' %(pulsar_name, kernel_name), samples)
